...
@section('content')
                    ...
                    @forelse ($invoices as $invoice)
                        <tr class="border-b">
                            ...
                            <td class="p-3 flex justify-center gap-2">
                                ...
                                <!-- GENERATE PDF -->
                                <a href="{{ route('staffinvoices.generate', $invoice->id) }}" target="_blank"
                    ...
                    @forelse ($receipts as $receipt)
                        <tr class="border-b">
                            ...
                            <td class="p-3 flex justify-center gap-2">
                                ...
                                <!-- GENERATE PDF -->
                                <a href="{{ route('staffreceipt.generate', $receipt->id) }}" target="_blank"
                                    class="table-action-btn bg-green-200 border border-green-400" title="Generate Invoice">
                                    <i class="fa-solid fa-download"></i>
                                </a>
    ...
    <script>
        ...
        function editInvoice(id) {
            fetch(`/staff/invoices/${id}/edit`)
                .then(res => res.json())
                .then(data => {

                    document.getElementById('editInvoiceForm')
                        .setAttribute('action', `/staff/invoices/${id}`);
    ...
    <script>
        ...
        function editReceipt(id) {
            fetch(`/staff/receipts/${id}/edit`)
                .then(res => res.json())
                .then(data => {

                    document.getElementById('editReceiptForm')
                        .setAttribute('action', `/staff/receipts/${id}`);
